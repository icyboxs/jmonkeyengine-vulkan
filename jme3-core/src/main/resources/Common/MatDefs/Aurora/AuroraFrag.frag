...
layout(set=0, binding=0, std140) uniform JmeUniforms {
	...
	vec4 m_Color;
    vec4 g_Resolution; // xy used
    vec4 g_Mouse;      // xy used
    vec4 g_Time;       // x used

};

layout(set=1, binding=0) uniform sampler2D m_ExtraTex;
...
void main(){
    ...
    vec2 q = fragCoord / g_Resolution.xy;
    vec2 p = q - 0.5;
    ...
    vec2 mo = g_Mouse.xy / g_Resolution.xy - 0.5;
    if (mo == vec2(-0.5)) mo = vec2(-0.1, 0.1);
    mo.x *= g_Resolution.x / g_Resolution.y;

    rd.yz *= mm2(mo.y);
    rd.xz *= mm2(mo.x + sin(g_Time.x * 0.05) * 0.2); // FIX
    ...
    if (rd.y > 0.0){
        ...
        float nz2 = triNoise2d((pos.xz + vec2(g_Time.x/10.0, g_Time.x/12.0)) * vec2(0.5, 0.7), 1.0); // FIX
        col += mix(vec3(0.2, 0.25, 0.5) * 0.08, vec3(0.3, 0.3, 0.5) * 0.7, nz2 * 0.4);
    }

	vec4 extra = texture(m_ExtraTex, texCoord1);
	col = mix(col, extra.rgb, 0.02); // 占比很小，避免画面突变
	fragColor = vec4(col, 1.0);
	//fragColor = texture(m_ExtraTex, vec2(1.0 - texCoord1.x, texCoord1.y));
	//fragColor = texture(m_ExtraTex, vec2(texCoord1.x, 1.0 - texCoord1.y));
	//fragColor = texture(m_ExtraTex, texCoord1);
}
