...
public final class DrawQueue {
    ...
    private final ArrayList<ComputeCmd> computeList = new ArrayList<>(128);

    public void enqueueCopy(CopyCmd cmd) { if (cmd != null && copyList.size() < 64) copyList.add(cmd); }
    public java.util.List<CopyCmd> getCopyCommands() { return copyList; }

    public void enqueueCompute(ComputeCmd cmd) { if (cmd != null && !isFull()) computeList.add(cmd); }
    public java.util.List<ComputeCmd> getComputeCommands() { return computeList; }

    public boolean isFull() { return (opaqueList.size() + guiList.size() + computeList.size()) >= MAX_QUEUED_DRAWS; }
    ...
    public void enqueue(DrawCmd dc) {
        if (dc != null && !isFull()) {
            ...
            if (isOpaque) { dc.submissionIndex = opaqueList.size(); opaqueList.add(dc); } 
            else { dc.submissionIndex = guiList.size(); guiList.add(dc); }
        }
    }
    ...
    public void clear() {
        ...
        for (int i = 0; i < copyList.size(); i++) copyList.get(i).recycle();
        for (int i = 0; i < computeList.size(); i++) computeList.get(i).recycle();
        copyList.clear(); opaqueList.clear(); guiList.clear(); computeList.clear();
    }
